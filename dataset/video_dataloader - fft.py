...
def extract_video(clip_path: str,
    ...
    for idx in frames_idx:
        ...
        try:
            image = read_image(frame_idx) / 255.0
            if image.shape[1] != 224 or image.shape[2] != 224:
                image = resize.forward(image)
            fft.append(get_multi_patch_fft_features(image))
            video.append(normalize.forward(image))
    ...
    fft = torch.stack(fft, dim=0)

    return video, fft


def get_multi_patch_fft_features(image,
                                 patch_sizes: list = [4, 8, 16, 32]):
    grayscale_image = rgb_to_grayscale(image)
    processed_image = grayscale_image.squeeze()
    H, W = processed_image.shape
    results = []
    for p_size in patch_sizes:
        patches = processed_image.unfold(0, p_size, p_size).unfold(1, p_size, p_size)
        fft_patches = torch.fft.fft2(patches)
        shifted_fft = torch.fft.fftshift(fft_patches, dim=(-2, -1))
        log_magnitude_patches = torch.log1p(torch.abs(shifted_fft))
        log_magnitude_patches = log_magnitude_patches.permute(0, 2, 1, 3).contiguous()
        feature_map = log_magnitude_patches.view(H, W)
        results.append(feature_map)
    return torch.stack(results, dim=0)


class ImageAudioDataset(Dataset):
    def __init__(self, config, dataset_info, mode='train'):
    ...
    def __getitem__(self, idx):
        ...
        label = sample['label']
        video, fft = extract_video(frame_path, frame_num, self.frames_per_segment, self.segments,
                              self.mode)
        return {'video': video, 'fft': fft, 'label': label}
