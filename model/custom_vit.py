...
class my_PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=32, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        self.img_size = img_size
...
class my_vit(nn.Module):
    ...
    def __init__(self, name='', pretrain_file=''):
        ...
        try:
            pretrained = torch.load(pretrain_file, map_location='cpu')['model']
        except FileNotFoundError:
            raise FileNotFoundError

        msg = self.v.load_state_dict(pretrained, strict=False)
        # print(msg)
        del pretrained
        # self.ViT.v.patch_embed.proj.weight

        ### -------> yb: custom forward (v0.6.7)
        my_conv = my_PatchEmbed(img_size=self.v.patch_embed.img_size[0], patch_size=self.v.patch_embed.patch_size[0],
                                in_chans=3, embed_dim=self.v.embed_dim)
    ...
    def forward_patch(self, x, is_shape_info=False):
        ...
        if self.v.pos_embed.size(1) != x.size(1):
            x = self.v.pos_drop(
                x + F.interpolate(self.v.pos_embed.permute(0, 2, 1), x.size(1), mode='linear').permute(0, 2, 1))
        else:
